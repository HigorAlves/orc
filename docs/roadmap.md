...
Skill/command bodies support `` !`command` `` preprocessing; `${CLAUDE_PLUGIN_ROOT}` substitutes inline. Today `flow`/`ship`/`plan`/`status`/`qa`/`resume`/`cleanup` carry `Bash(. */lib/workspace-detect.sh*)` allowed-tools wildcards and burn a model round-trip sourcing `lib/workspace-detect.sh` at runtime — and the lib's memoization (`ORC_CONTEXT_CACHED`) never works because every Bash call is a fresh subshell. Add a `--banner` CLI mode to the lib and inject `` Workspace: !`"${CLAUDE_PLUGIN_ROOT}"/lib/workspace-detect.sh --banner` `` at the top of those command files; delete the dot-source patterns from `allowed-tools`. `/orc:status` can inject the whole `.orc/orc.json` registry the same way.
...
The mirror is broken beyond cosmetics: `antigravity/hooks.json` invokes nonexistent `gemini-skills/` paths, `hooks/session-start.sh` reads a nonexistent `orc-core/` skill (injects a literal error string), versions triple-drift (0.4.0 / 0.4.3 / 0.5.3), ~70 files differ from `orc/`, and `dist/` has no generator. Either (a) write `scripts/sync-antigravity.sh` + CI byte-compare and fix the two broken hook paths, or (b) move `antigravity/` to its own repo/branch and delete it here. Given zero shared tooling exists, (b) is cheaper and honest. Decide early — it de-risks every future change.
