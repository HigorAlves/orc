...
<p align="center">
  <code>79 skills</code> &nbsp;·&nbsp; <code>30 commands</code> &nbsp;·&nbsp;
...
> [!TIP]
> Pin a specific version: `orc install --ref orc--v0.18.0` (any existing release tag, or a commit SHA). Using `--ref` writes `settings.json` directly.
...
├── skills/<name>/SKILL.md       # 79 skills — a thin index per skill
...
│                                # pre-git-guard.sh (one prompt per command)
