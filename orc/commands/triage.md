...
2. **Recommend.** Print the Gate headline (`**⛔ Gate — triage recommendation**`, one line: category + state + why, per `orc:callouts`), then `AskUserQuestion`: accept the recommendation / pick a different state / verify the claim first. Include a brief codebase summary relevant to the request — including whether it's already implemented.
