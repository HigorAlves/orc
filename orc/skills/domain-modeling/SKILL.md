...
If any of the three is missing, skip the ADR. In orc, ADRs are authored via the `orc:adr-writing` template into `docs/adr/NNNN-*.md` — see [ADR-FORMAT.md](./references/ADR-FORMAT.md) for the lightweight shape and what qualifies, and `orc:adr-writing` for the full house template.
